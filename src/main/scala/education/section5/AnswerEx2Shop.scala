 ...
 */
  ...
  def findCustomerName(byId: Map[Customer.Id, Customer], id: Customer.Id): String =
    byId
      .get(id)                        // 顧客idを取り出す
      .map(customer => customer.name) // 顧客idから顧客名を取り出す
 ...
 */
  def totalByCustomer(orders: Seq[Order]): Map[Customer.Id, Int] =
    val groupedOrdersByCustomer =                                // オーダーした顧客id
      orders                                                     // 全ての注文情報
        .filter(order => order.status != Order.Status.Cancelled) // オーダーステイタスがキャンセル以外のもを抽出
        .groupBy(order => order.customerId)                      // 顧客idでまとめる

    groupedOrdersByCustomer                                       // オーダーした顧客id
      .view.mapValues(ordersForCustomer => ordersForCustomer
 ...
 */
  def neverOrdered(customers: Seq[Customer], orders: Seq[Order]): Set[String] =
    val orderedCustomerIds =            // 注文したことある顧客id
      orders                            // オーダー履歴
        .map(order => order.customerId) // オーダー履歴からカスタマーIDを取り出す(Seq(id))
        .toSet                          // setに変換し重複をなしに(Set(id))

    val allCustomerIds =              // 顧客全員id
      customers                       // 顧客情報
        .map(customer => customer.id) // 顧客情報からカスタマーIDを取り出す(Seq(id))
        .toSet                        // setに変換し重複をなしに(Set(id))

    val neverOrderedCustomerIds = // 注文したことない顧客id
      allCustomerIds              // 顧客全員id
        .diff(orderedCustomerIds) // 顧客全員id - 注文したことある顧客id

    customers                                                            // 顧客情報
      .filter(customer => neverOrderedCustomerIds.contains(customer.id)) // 顧客情報から注文したことない顧客を取り出す
