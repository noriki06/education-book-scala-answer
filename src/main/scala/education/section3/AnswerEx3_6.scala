...
object AnswerEx3_6:
  ...
  //def takeTurn(attacker: Seq[Pokemon], defender: Seq[Pokemon]): (Seq[Pokemon], Seq[Pokemon]) =
    //val attackerPokemon =　　　　　　　　　// 攻撃するポケモン
      //attacker                             // 攻撃側
        //.filter(pokemon => pokemon.hp > 0) // hpが0以上
        //.head                              // 先頭

    //val defenderPokemon =                  // 防御するポケモン
      //defender                             // 防御側
        //.filter(pokemon => pokemon.hp > 0) // hpが0以上
        //.head                              // 先頭
    ...
    //val randomIndex =                        // ランダムなインデックスを取得
      //MATH_RANDOM                            // ランダムな数値
        //.nextInt(attackerPokemon.skill.size) // 技の数

    //val attackSkill =       // ランダムなインデックスで技を決定
      //attackerPokemon       // 攻撃するポケモン
        //.skill(randomIndex) // 技をランダムな数値で決定

    //val newDefenderHp =                                 // 攻撃された後の防御側のポケモンのhp
      //scala
        //.math
        //.max(0, defenderPokemon.hp - attackSkill.power) // 0 or hp - power

    //val updatedDefenderPokemon =  // 防御側のポケモンを更新
      //defenderPokemon             // 防御側ポケモン
        //.copy(hp = newDefenderHp) // hpを更新してインスタンス生成

    //val newAttackerHp =                                                     // 攻撃された後の防御側のポケモンのhp
      //scala
        //.math
        //.min(attackerPokemon.hpMax, attackerPokemon.hp + attackSkill.power) // hpMax or hp + 回復power

    //val updatedAttackerPokemon =  // 防御側のポケモンを更新
      //attackerPokemon             // 攻撃側のポケモン
        //.copy(hp = newAttackerHp) // hpを更新してインスタンス生成

    //val updatedDefenderTeam =                   //防御側のSeq[pokemon]の更新
