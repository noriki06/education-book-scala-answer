...
class Answer5Controller @Inject()(edu: EduRepositoryFacade)(using ExecutionContext):
  def invoke(): Future[Unit] =
    ...
    // ② 取得：ID を持つ EmbeddedId が Option で返る
      ...
      total <- users.foreach(u => println(s"${u.v.name}：${u.v.state}"))
      deleteId <- total.filter(_.v.state == User.Status.Withdrawn).map(_.id)
      deleted <- edu.user.delete(Seq(deleteId))
      after <- users.foreach(u => println(s"${u.v.name}：${u.v.state}"))
    } yield (after)
