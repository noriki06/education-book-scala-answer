...
object AnswerEx1:
  ...
  def executeTransfer(senderId: Int, receiverId: Int, transferAmount: Int, accountMap: Map[Int, Account]):
    ...
    for {
      a <- EitherT(searchAccountById(accountMap, senderId))
      b <- returnBalance(a, transferAmount)
      c <- EitherT(searchAccountById(accountMap, receiverId))
    } yield (b, returnBalance(c, -transferAmount))
